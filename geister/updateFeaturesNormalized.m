...
function gameState = updateFeaturesNormalized(gameState)
...
for i = 1:length(gameState.ghosts.evil)
    ...
    % Set features for the closest opponent ghost
    thisGhost.features.ClosestGhostDist = opponentDistances(ind); % scale 0-10
    thisGhost.features.ClosestGhostGoodConf = opponentGoodConf(ind);
    thisGhost.features.ClosestGhostEvilConf = opponentEvilConf(ind);
    thisGhost.features.ClosestGhostRow = opponentRow(ind);
    thisGhost.features.ClosestGhostCol = opponentCol(ind);
    ...
    thisGhost.features.DistToOpponentExit = distToOpponentExit(thisGhost);%0-8
    thisGhost.features.ExitCongestion = exitCongestion(thisGhost, gameState.board);%0-8
    ...
    thisGhost.features.OpponentDistToHomeExit = oppDistancesToHomeExit(ind,1);% scaled from 0-8
    thisGhost.features.OpponentClosestToExitLR = oppDistancesToHomeExit(ind,2);% scaled from 0-10
    thisGhost.features.DistToHomeExit = distToBlockExit(thisGhost, oppDistancesToHomeExit(ind,2));% scaled from 0-8
