...
function gameState = initializeBoardPlayer(fig)

% % Pick a random initial position for our own ghosts
% initialPosition = reshape(randperm(8) > 4, 2, 4);

board = zeros(6);
board(1:2, 2:5) = repmat(3, 2,4);
fig = drawBoard(board, fig);

% Get good ghost positions
for i = 1:4

    valid = false;
    while ~valid

        disp(['Pick position for good ghost ' num2str(i)]);
        pos = getPosition(fig);
        col = pos(1);
        row = pos(2);

        %Check validity
        if col >= 1 && col <= 4 && row >= 0 && row <= 1
...
% Get evil ghost positions
for i = 1:4

    valid = false;
    while ~valid

        disp(['Pick position for evil ghost ' num2str(i)]);
        pos = getPosition(fig);
        col = pos(1);
        row = pos(2);
...
initialPosition = flipud(board(5:6, 2:5));
...
[row, col] = find(initialPosition == 2);
for i = 1:4
    thisGhost = [];
